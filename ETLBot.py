...
class Command:
    ...
    # Блок текста Помощи
    message_help = (f"Описание возможностей и функций кнопок 📑\n"
                          ...
                          f"| {next_word.text} | используйте для перехода к следующей странице слов.\n"
                          f"| {add_word.text} | добавляет слово на 🇷🇺 и 🏴󠁧󠁢󠁥󠁮󠁧󠁿 языке"
                          "в вашу личную базу знаний.\n"
                          f"| {my_word.text} | показать список добавленных Вами слов \n"
                          f"| {my_experience.text} | вывод статистики успешных сопоставлений слов\n"
    ...
    #Блок о добавлении слов
    messages_add_word = [f"Для добавления слова Вам необходимо ввести слово и его перевод. \n"
                         f"Ввод слова производите с заглавной буквы",
                         f"Введите слово на русском языке и отправьте сообщение: 📝",
                         f"Введите перевод слова на английском языке и отправьте сообщение: 📝"
                         ]
    #Блок теста проверки добавляемого слова
    check_add_word = [f"Внимательнее к требованиям заполнения! "
                      f"Слова начинаются с Заглавной буквы."
                      f"Слово должно быть введено на русском языке",
                      f"Внимательнее к требованиям заполнения! "
                      f"Слова начинаются с Заглавной буквы."
...
def send_welcome(message):
    ...
    bot.reply_to(message, f"Здравствуйте, уважаемый {user_info} 👋 \n"
                          ...
                          f"База содержит - {Command.count_word} слова🧡 \n"
...
def help_message(message):
    """Всё просто это HELP"""
    bot.reply_to(message, Command.message_help)
...
def start_of_studies(message):
    ...
    random.shuffle(Command.message_question)
    bot.send_message(message.chat.id, f"{Command.message_question[0]} \n ⭐ {rus_word} ⭐:", reply_markup=markup)
...
def add_word(message):
    ...
    if Addword.id_add_word == 0:
        bot.send_message(message.chat.id, Command.messages_add_word[0],
                         reply_markup=markup)
        bot.send_message(message.chat.id, Command.messages_add_word[1])
    else:
        bot.send_message(message.chat.id, Command.messages_add_word[2], reply_markup=markup)
...
def send_add_word(message):
    ...
    if message.text == Command.back.text:
        ...
        if (len(message.text) < 2 or message.text[0].islower()
            ...
            bot.send_message(message.chat.id, Command.check_add_word[0])
            Addword.id_add_word = 0
            start_of_studies(message)
        else:
        ...
        if (len(message.text) < 2 or message.text[0].islower()
            ...
            bot.send_message(message.chat.id, Command.check_add_word[1])
            Addword.id_add_word = 0
            start_of_studies(message)
        else:
...
@bot.message_handler(func=lambda message: message.text == Command.next_word.text or message.text == Command.back.text)
...
def message_reply(message):
    ...
    Addword.id_user_word = message.from_user.id
    Addword.new_eng_word = message.text
    random.shuffle(Command.message_win)
    random.shuffle(Command.message_lose)
    try:
        if message.text == result[0]:
            bot.send_message(message.chat.id, f"{Command.message_win[0]}")
            bot.send_message(message.chat.id, f"{Command.go_next}")
            BaseETLBot.add_experience(Addword.id_user_word, Addword.new_eng_word)
            start_of_studies(message)
        else:
            bot.send_message(message.chat.id, f"{Command.message_lose[0]}")
    except:
        start_of_studies(message)
